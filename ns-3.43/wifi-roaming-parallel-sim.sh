...
run_simulations() {
    ...
    for seed in {1..2}; do
        echo "Running simulation with file=$simulation_file, maxMissedBeacons=$maxMissedBeacons, seed=$seed, reliabilityMode=$reliabilityMode"
        ./ns3 run "$simulation_file" -- --seed=$seed --maxMissedBeacons=$maxMissedBeacons --poissonLambda=$poissonLambda --reliabilityMode=$reliabilityMode --wallLoss=$wallLoss --printOutput=$printOutput
    done
}
