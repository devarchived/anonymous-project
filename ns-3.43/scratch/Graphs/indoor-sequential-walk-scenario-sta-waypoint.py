...
waypoints_file = os.path.expanduser(
    '~/Desktop/Workspace/ns-allinone-3.43/ns-3.43/scratch/wifi-ehr-roaming-scenario/random-human-waypoints.txt'
)
waypoints = []
...
# Plot dashed path with arrows
if len(waypoints) > 1:
    xs, ys = zip(*waypoints)
    # Draw dashed path
    plt.scatter(*zip(*waypoints), marker='.', s=5, c='blue', edgecolor='black', linewidths=0.5, zorder=1)
    # Draw arrows between waypoints
    for i in range(len(waypoints)-1):
        plt.annotate(
            '', xy=waypoints[i+1], xytext=waypoints[i],
            arrowprops=dict(
                arrowstyle='-|>', 
                color='blue', 
                lw=1, 
...
# Plot APs
plt.scatter(*zip(*ap), marker='D', s=50, c='red', edgecolor='black', linewidths=1.5, label='AP')
plt.scatter(*zip(*sta), marker='o', s=25, c='blue', edgecolor='black', linewidths=0.5, label='STA')
...
# Save as PDF
# plt.show()
plt.savefig('indoor-sequential-walk-scenario-sta-waypoint.pdf',
           bbox_inches='tight',
           dpi=1200,
    	   format='pdf')
plt.close()

print("Plot saved as 'indoor-sequential-walk-scenario-sta-waypoint.pdf'")
