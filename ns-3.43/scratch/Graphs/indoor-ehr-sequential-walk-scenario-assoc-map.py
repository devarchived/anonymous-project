...
log_file = os.path.expanduser(
    '~/Desktop/Workspace/ns-allinone-3.43/ns-3.43/scratch/wifi-ehr-roaming-scenario/assoc-rxpower-logs/wifi-ehr-roaming-assoc-unsaturated-beacons-4-wall-loss-0-seed-21.txt'
)
...
if "wifi-ehr-roaming-assoc-" in base_name:
    scenario_part = base_name.split("wifi-ehr-roaming-assoc-")[1].replace(".txt", "")
else:
...
# Plot APs (diamonds)
plt.scatter(*zip(*ap1), marker='D', s=75, c='#90EE90', edgecolor='black', linewidths=1.5, label='AP 5 GHz')
plt.scatter(*zip(*ap2), marker='D', s=75, c='green', edgecolor='black', linewidths=1.5, label='AP 6 GHz')
plt.scatter(*zip(*ap3), marker='D', s=75, c='blue', edgecolor='black', linewidths=1.5, label='AP 2.4 GHz')
...
wall_color = 'gray'
plt.gca().add_patch(Rectangle((0, 0), building_wall_thickness, W, color=wall_color))
plt.gca().add_patch(Rectangle((L - building_wall_thickness, 0), building_wall_thickness, W, color=wall_color))
plt.gca().add_patch(Rectangle((0, 0), L, building_wall_thickness, color=wall_color))
plt.gca().add_patch(Rectangle((0, W - building_wall_thickness), L, building_wall_thickness, color=wall_color))
