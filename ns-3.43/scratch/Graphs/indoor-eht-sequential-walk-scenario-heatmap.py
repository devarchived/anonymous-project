...
log_file = os.path.expanduser(
    '~/Desktop/Workspace/ns-allinone-3.43/ns-3.43/scratch/wifi-eht-roaming-scenario/assoc-rxpower-logs/wifi-eht-roaming-rxPower-unsaturated-beacons-4-wall-loss-0-seed-21.txt'
)
...
if "wifi-eht-roaming-rxPower-" in base_name:
    scenario_part = base_name.split("wifi-eht-roaming-rxPower-")[1].replace(".txt", "")
else:
...
while i < len(lines):
    ...
    if line.startswith("New position:"):
        ...
        rxpowers = []
        # Read RxPower lines until next New position or end of file
        while i+1 < len(lines) and lines[i+1].startswith("RxPower"):
            i += 1
            rx_line = lines[i]
            rx_val = float(rx_line.split(":")[-1].replace("dB", "").strip())
            rxpowers.append(rx_val)
        if rxpowers:
            max_rx = max(rxpowers)
            heatmap_x.append(x)
            heatmap_y.append(y)
            heatmap_val.append(max_rx)
            heatmap_color.append(None)  # Use colormap
        else:
            # No RxPower lines after position: treat as dead zone
            heatmap_x.append(x)
            heatmap_y.append(y)
