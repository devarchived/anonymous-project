...
def calculate_utilization_factor_mlo(lambda_total, mu, n_links):
...
def calculate_steady_state_0_mlo(rho, n_links):
    #rho = np.clip(rho, 0.0001, 0.9999)
    sum_1 = 0
...
def calculate_steady_state_k_mlo(rho,k, n_links):
    pi_0 = calculate_steady_state_0_mlo(rho, n_links)
    if k >= n_links :
...
def calculate_mean_backoff_time_mlo(rho, n_links, mean_backoff_slot):
    rho = np.clip(rho, 0.0001, 0.9999)
    sum_1 = 0
    for n in range (n_links-1) :
        sum_1 += calculate_steady_state_k_mlo(rho,n, n_links)*(mean_backoff_slot)/(n_links-n+1)

    sum_2 = (mean_backoff_slot/2)*calculate_steady_state_0_mlo(rho, n_links)*((n_links**n_links)/math.factorial(n_links))*((rho**n_links)/(1-rho))
...
def calculate_mean_waiting_time_mlo(rho, n_links, lambda_total):
    rho = np.clip(rho, 0.0001, 0.9999)
    p_busy = ((n_links*rho)**n_links)*calculate_steady_state_0_mlo(rho, n_links)/(math.factorial(n_links)*(1-rho))
...
def calculate_tau_mlo(rho, n_links, mean_backoff_time):
    """Calculate the transmission probability tau."""
    diff = 0
    for i in range (n_links-1) :
        diff += (n_links-i)/n_links*calculate_steady_state_k_mlo(rho,i, n_links)
    gamma = 1 - diff
...
def calculate_throughput(n_obss, p_transmit, p_success, p_error, packet_length, data_rate, slot_size, t_success, t_collision, t_error, rho, mu, packet_size):
    ...
    th = (1-p_error)*rho * mu * packet_size * 8
    return th

def calculate_throughput_mlo(n_obss, p_transmit, p_success, p_error, packet_length, data_rate, slot_size, t_success, t_collision, t_error, rho, mu, packet_size, n_links):
    """Calculate the throughput."""
...
def run_simulation_for_band_mlo(band_params, common_params):
    """Run the simulation for a specific frequency band."""
    # Unpack common parameters
    alpha = common_params['alpha']
    eps = common_params['eps']
    ...
    m = common_params['m']
    M = common_params['M']
    n_links = common_params['n_links'] - common_params['n_error_links']
    data_rate = common_params['data_rate']
    min_data_rate = common_params['min_data_rate']
    lambda_poisson = common_params['lambda_poisson']
    phy_header = common_params['phy_header']
    ...
    for it in range(max_iter):
        ...
        mean_backoff_slot = calculate_mean_backoff_slot(p, m, M, w_0)
        mean_backoff_time = calculate_mean_backoff_time_mlo(rho, n_links, mean_backoff_slot)
        ...
        rho_new = calculate_utilization_factor_mlo(lambda_total, mu, n_links)
        tau_new = calculate_tau_mlo(rho, n_links, mean_backoff_time)
    ...
    mean_service_time = calculate_mean_service_time(p, p_collision, band_params.p_error, m, delta, slot_size, t_collision, t_error, mean_backoff_time, t_success)
    mu = calculate_mu(mean_service_time)
    mean_waiting_time = calculate_mean_waiting_time_mlo(rho, n_links, lambda_total)
    ...
    throughput = calculate_throughput_mlo(band_params.n_obss, p_transmit, p_success, band_params.p_error, packet_length, data_rate, slot_size, t_success, t_collision, t_error, rho, mu, packet_size, n_links)
    reliability = calculate_reliability(p, M)
    delay = calculate_delay(mean_service_time, mean_waiting_time)
...
def run_simulation_for_error_band_mlo(band_params, common_params):
    """Run the simulation for a specific frequency band."""
    # Unpack common parameters
    alpha = common_params['alpha']
    eps = common_params['eps']
    ...
    m = common_params['m']
    M = common_params['M']
    n_links = common_params['n_error_links']
    data_rate = common_params['data_rate']
    min_data_rate = common_params['min_data_rate']
    lambda_poisson = common_params['lambda_poisson']
    phy_header = common_params['phy_header']
...
def calculate_final_metrics(results):
    ...
    for result in results:
        ...
        if result['delay'] != None and result['delay'] < min_delay:
            min_delay = result['delay']
    
    # Calculate averages
    num_bands = len(results)
    avg_reliability = reliability_sum / num_bands if num_bands > 0 else 0
    avg_delay = delay_sum / (num_bands-error_count) if (num_bands-error_count) > 0 else 0
...
def calculate_final_metrics_mlo(results):
    """Calculate aggregate metrics from all bands' simulation results."""
    # Initialize metrics
    total_throughput = 0
    reliability_sum = 0
...
def run_analysis_for_mlo_n_obss(n_obss_values, bands_template, common_params):
    """Run simulations for different n_obss values and collect metrics."""
    result_summary = {
        'total_throughput': [],
        'avg_reliability': [],
...
def plot_throughput(n_obss_values, result_summary, avg_total_th_mlo):
    """Plot throughput against n_obss values."""
    
    plt.figure(figsize=(8, 5))
    
    plt.plot(n_obss_values, [t/1e6 for t in result_summary['total_throughput']], 'bo-', label='WiFi 8 JT')
    plt.plot(n_obss_values, [t/1e6 for t in result_summary['max_throughput']], 'go', label='WiFi 8 JT Reliability Mode')
    plt.plot(n_obss_values, [t/1e6 for t in avg_total_th_mlo], 'r+--', label='WiFi 7 MLO - STR')
...
def plot_reliability(n_obss_values, result_summary, avg_reliability):
    """Plot reliability against n_obss values."""
    
    plt.figure(figsize=(8, 5))
    
    plt.plot(n_obss_values, [r*100 for r in result_summary['avg_reliability']], 'bo-', label='WiFi 8 JT')
    plt.plot(n_obss_values, [r*100 for r in result_summary['jt_reliability']], 'go', label='WiFi 8 JT Reliability Mode')
    plt.plot(n_obss_values, [r*100 for r in avg_reliability], 'r+--', label='WiFi 7 MLO - STR')
...
def plot_delay(n_obss_values, result_summary, mlo_result_summary):
    """Plot delay against n_obss values."""
    
    plt.figure(figsize=(8, 5))
    
    plt.plot(n_obss_values, [d*1e3 for d in result_summary['avg_delay']], 'bo-', label='WiFi 8 JT')
    plt.plot(n_obss_values, [d*1e3 for d in result_summary['min_delay']], 'go', label='WiFi 8 JT Reliability Mode')
    plt.plot(n_obss_values, [d*1e3 for d in mlo_result_summary['avg_delay']], 'r+--', label='WiFi 7 MLO - STR')
...
def main():
    ...
    n_obss_values = list(range(1, 5))
    result_summary = run_analysis_for_n_obss(n_obss_values, bands, common_params)
    mlo_result_summary = run_analysis_for_mlo_n_obss(n_obss_values, bands_mlo, common_params)
    error_mlo_result_summary = run_analysis_for_mlo_n_obss(n_obss_values, bands_mlo_error, common_params)

    avg_total_th_mlo = (n_error_links * np.array(error_mlo_result_summary['total_throughput']) +
    n_normal_links * np.array(mlo_result_summary['total_throughput'])
    ) / n_links

    avg_reliability_mlo = (
    n_error_links * np.array(error_mlo_result_summary['avg_reliability']) +
    n_normal_links * np.array(mlo_result_summary['avg_reliability'])
    ) / n_links
    ...
    rel_data = [filter_extreme_values(wifi_ehr[wifi_ehr['numBSS'] == val]['PacketReceivedReliability'].values) for val in sorted(wifi_ehr['numBSS'].unique())]
    positions = sorted(wifi_ehr['numBSS'].unique())
    ...
    # Plot reliability (Simulation vs Analysis)
    rel_data = [filter_extreme_values(wifi_ehr_rel[wifi_ehr_rel['numBSS'] == val]['PacketReceivedReliability'].values) for val in sorted(wifi_ehr_rel['numBSS'].unique())]
    positions = sorted(wifi_ehr_rel['numBSS'].unique())
    ...
    # plot_reliability(n_obss_values,result_summary, avg_reliability_mlo)
    # plot_delay(n_obss_values,result_summary, mlo_result_summary)
