...
def run_analysis_for_n_obss(n_obss_values, bands_template, band_template_mlo, common_params):
    """Run simulations for different n_obss values and collect metrics."""
    result_summary = {
        'total_throughput': [],
        'max_throughput': [],
        'total_throughput_mlo': [],
        'avg_reliability': [],
        'jt_reliability' : [],
        'avg_reliability_mlo': [],
        'avg_delay': [],
    ...
    for n in n_obss_values:
        ...
        bands_mlo = [
            BandParameters(name=b.name, frequency=b.frequency, 
                         difs=b.difs, sifs=b.sifs, n_obss=n, p_error=b.p_error)
            for b in band_template_mlo
        ]
        
        # Run simulations
        results = [run_simulation_for_band(band, common_params) for band in bands]
        results_mlo = [run_simulation_for_band_mlo(band, common_params) for band in bands_mlo]

        # Calculate aggregate metrics
        temp = calculate_final_metrics(results)
        temp_mlo = calculate_final_metrics_mlo(results_mlo)
        ...
        result_summary['total_throughput_mlo'].append(temp_mlo['total_throughput'])
        result_summary['avg_reliability_mlo'].append(temp_mlo['avg_reliability'])
        result_summary['avg_delay_mlo'].append(temp_mlo['avg_delay'])
...
def plot_throughput(n_obss_values, result_summary):
    ...
    plt.plot(n_obss_values, [t/1e6 for t in result_summary['total_throughput_mlo']], 'r+--', label='WiFi 7 MLO - STR')
...
def plot_reliability(n_obss_values, result_summary):
    ...
    plt.plot(n_obss_values, [r*100 for r in result_summary['avg_reliability_mlo']], 'r+--', label='WiFi 7 MLO - STR')
...
def plot_delay(n_obss_values, result_summary):
    ...
    plt.plot(n_obss_values, [d*1e3 for d in result_summary['avg_delay_mlo']], 'r+--', label='WiFi 7 MLO - STR')
...
def main():
    ...
    result_summary = run_analysis_for_n_obss(n_obss_values, bands, bands_mlo, common_params)
    ...
    th_analysis = plt.plot(n_obss_values, [t/1e6 for t in result_summary['total_throughput_mlo']], 'go-', label='WiFi 7 MLO-STR (Analysis)')
    ...
    rel_box_label = mpatches.Patch(facecolor='white', edgecolor='black', 
                          label='WiFi 7 MLO-STR (ns-3 Simulation)')
    rel_analysis = plt.plot(n_obss_values, [r*100 for r in result_summary['avg_reliability_mlo']], 'go-', label='WiFi 7 MLO-STR (Analysis)')
    ...
    delay_box_label = mpatches.Patch(facecolor='white', edgecolor='black', 
                          label='WiFi 7 MLO-STR (ns-3 Simulation)')
    delay_analysis = plt.plot(n_obss_values, [d*1e3 for d in result_summary['avg_delay_mlo']], 'go-', label='WiFi 7 MLO-STR (Analysis)')
