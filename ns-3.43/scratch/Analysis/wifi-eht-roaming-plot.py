...
def main():
    ...
    wifi_eht['AssociationDelay'] = (wifi_eht['AssociationDelay'].str.replace('ms', '').str.replace('+', '').astype(float) / 1000
)
    ...
    th_data = [wifi_eht[wifi_eht['maxMissedBeacons'] == val]['Throughput'].values for val in sorted(wifi_eht['maxMissedBeacons'].unique())]
    ...
    rel_data = [wifi_eht[wifi_eht['maxMissedBeacons'] == val]['PacketReceivedReliability'].values for val in sorted(wifi_eht['maxMissedBeacons'].unique())]
    positions = sorted(wifi_eht['maxMissedBeacons'].unique())
    ...
    delay_data = [filter_extreme_values(wifi_eht[wifi_eht['maxMissedBeacons'] == val]['ChannelAccessDelay'].values) for val in sorted(wifi_eht['maxMissedBeacons'].unique())]
    positions = sorted(wifi_eht['maxMissedBeacons'].unique())
    ...
    # Plot delay (Simulation vs Analysis)
    assoc_delay_data = [filter_extreme_values(wifi_eht[wifi_eht['maxMissedBeacons'] == val]['AssociationDelay'].values) for val in sorted(wifi_eht['maxMissedBeacons'].unique())]
    positions = sorted(wifi_eht['maxMissedBeacons'].unique())
