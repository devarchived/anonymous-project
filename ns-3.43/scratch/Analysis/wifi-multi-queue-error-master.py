...
def run_simulation_for_error_band(band_params, common_params):
    """Run the simulation for a specific frequency band."""
    # Unpack common parameters
    alpha = common_params['alpha']
    eps = common_params['eps']
...
def main():
    ...
    error_result_summary = run_analysis_for_n_obss(n_obss_values, bands_error, common_params)

    avg_total_th = (n_error_links * np.array(error_result_summary['total_throughput']) +
    n_normal_links * np.array(result_summary['total_throughput'])
    ) / n_links

    avg_reliability = (
    n_error_links * np.array(error_result_summary['avg_reliability']) +
    n_normal_links * np.array(result_summary['avg_reliability'])
    ) / n_links
    ...
    th_analysis = plt.plot(n_obss_values, [t/1e6 for t in avg_total_th], 'go-', label='WiFi 7 MLO-STR (Analysis)')
    ...
    rel_box_label = mpatches.Patch(facecolor='white', edgecolor='black', 
                          label='WiFi 7 MLO-STR (ns-3 Simulation)')
    rel_analysis = plt.plot(n_obss_values, [r*100 for r in avg_reliability], 'go-', label='WiFi 7 MLO-STR (Analysis)')
