...
main(int argc, char* argv[])
{
    ...
    LogComponentEnable("eht-wifi-network", LOG_LEVEL_ALL);
    ...
    // Debug number of links
    std::cout << "Number of links : " << (int)nLinks << std::endl;
    ...
    auto serverNodes2 = downlink ? std::ref(wifiStaNodes) : std::ref(wifiApNode2);
    Ipv4InterfaceContainer serverInterfaces;
    Ipv4InterfaceContainer serverInterfaces2;
    NodeContainer clientNodes;
    for (std::size_t i = 0; i < nStations; i++)
    {
        serverInterfaces2.Add(downlink ? staNodeInterfaces.Get(i)
                                        : apNodeInterface2.Get(0));
        // serverInterfaces.Add(downlink ? staNodeInterfaces.Get(i)
        //                                 : apNodeInterface.Get(1));
        clientNodes.Add(downlink ? wifiApNode2.Get(0) : wifiStaNodes.Get(i));
    }
    ...
    if (udp)
    {
        ...
        // serverApp.Add(server.Install(serverNodes.get()));
        ...
        for (std::size_t i = 0; i < nStations; i++)
        {
            ...
            UdpClientHelper client(serverInterfaces2.GetAddress(i), port);
